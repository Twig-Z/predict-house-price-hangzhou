...
class HtmlParser():
    ...
    def get_ershoufang_data(self, html_cont, id):
        ...
        tag_area = bsObj.find("div", {"class": "areaName"}).find("span", {"class": "info"}).find_all("a")
        if tag_area is not None:
            areaName = tag_area[0].get_text()
            placeName = tag_area[1].get_text()
            #print(placeName)
        else:
            areaName = "null"
            placeName = "null"
        ...
        tag_supplement = bsObj.find("a", {"class": "supplement"})
        if tag_supplement is not None:
            supplement = tag_supplement.get_text()
            #print(supplement)
        else:
            supplement = "无地铁"
            self.log.logger.error("页面解析(detail)：找不到supplement标签！")
        ...
        ershoufang_data.append(placeName)
        ershoufang_data.append(supplement)
