...
class SpiderMain():
    ...
    def craw(self, root_url):
        ...
        # 杭州各区域area和拉取页面参考
        """areas = {
                        "xihuqu4":100, “qiantangqu”：100，“linpingqu”：100，
                        “gongshuqu":100, "shangchengqu":100,"binjiangqu":100,
                        "yuhangqu":100, "xiaoshanqu":100,"tongluxian":100,
                        "fuyangqu":100,"linanqu":100, 
                        }
                """
        area = "linanqu"  # 你要爬取的城区

        pg_sum = 51  # 从pg_sum爬到pg_sum+20页
        self.outputer.file_name(area, pg_sum)

        # 1、抓取所有二手房详情界面链接，并将所有连接放入URL管理模块
        # for num in range(1, pg_sum + 1):
        for num in range(pg_sum, pg_sum + 70):
            # 1.1 拼接页面地址: https://hz.lianjia.com/ershoufang/yanqing/pg2/
            ...
            try:
                ...
                try:
                    ...
                    # 暂停10-20秒的整数秒，时间区间：[10,20]
                    zzz = random.randint(5, 10)
                    print("休息" + str(zzz) + "秒")
                    time.sleep(zzz)
        # 2、解析二手房具体细心页面
        id = 1
...
if __name__ == "__main__":
    # 设定爬虫入口URL
    root_url = "https://hz.lianjia.com/ershoufang/"
    # 初始化爬虫对象
